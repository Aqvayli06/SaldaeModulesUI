...
SA_key_figures_Server <- function(input, output, session,tisefka) {
  ...
  output$SA_key_figure <- renderUI({
    ...
    shinydashboard::infoBoxOutput(session$ns("SA_infor_box"))
  })
  output$SA_infor_box <- shinydashboard::renderInfoBox({
    if(input$SA_key_figure_select=="None")return(NULL)
    shinydashboard::infoBox(title = input$SA_key_figure_select,
