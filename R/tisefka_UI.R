...
SA_tisefka_mod <- function(input, output, session,tisefka) {
  ...
  output$tisefka_table <- DT::renderDataTable({
    ...
    DT::datatable(tisefka_tizegzawin()%>%dplyr::select(!!input$variable_picker),extensions = 'Scroller', options = list(deferRender = TRUE, scrollY = 200, scroller = TRUE))
...
tisefka_tables <- reactive({
    ...
    a <- purrr::map(a,~DT::datatable(.x,extensions = 'Scroller', options = list(deferRender = TRUE, scrollY = 200, scroller = TRUE)))%>%
...
#---------------------------------------
  ...
  tisefka_tables <- reactive({
    ...
    return(purrr::map(.x = tisefka_aggregated()$tisefka,~DT::datatable(.x,extensions = 'Scroller', options = list(deferRender = TRUE, scrollY = 200, scroller = TRUE)))%>%
