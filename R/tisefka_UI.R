...
SA_tisefka_mod <- function(input, output, session,tisefka) {
  ...
  output$tisefka_view_box <- renderUI({
    shinydashboard::box(title = "Exploration Board",collapsible = TRUE,
  ...
  output$tisefka_table <- DT::renderDataTable({
    req(input$variable_picker)
    DT::datatable(tisefka_iheggan(),extensions = 'Scroller', options = list(deferRender = TRUE, scrollY = 200, scroller = TRUE))
