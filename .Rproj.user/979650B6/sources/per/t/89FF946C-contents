...
SA_reporting_UI <- function(id){
  ...
  fluidPage(
    ...
    fluidRow(
      column(width = 4,
             textOutput(ns("report_header_inf"))
      )
    ),
    #------------------- Generate Report
...
SA_reporting_mod <- function(input, output, session){
...
output$report_header_inf <- renderText({
  paste("Report Header Info length:",saldae_report_header())
})
...
observeEvent(input$generate_report,{
  ...
  SaldaeReporting::SALDAE_reporting_engine(dash_aqerruy = saldae_report_header(),dash_ul = dash_ul())
})
